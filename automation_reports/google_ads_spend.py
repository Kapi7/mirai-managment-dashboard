...
def _query_hourly_cost(client: GoogleAdsClient, customer_id: str,
    ...
    try:
        ...
        """
        yield from ga_service.search(request=req)
    finally:
        ga_service.transport.close()
...
def _sum_accounts_usd_aligned(day_iso: str, shop_tz: str, config_path: str,
    ...
    for cid in ids:
        try:
            ...
            # One inaccessible/legacy account (e.g. an old id left in
            # GOOGLE_ADS_CUSTOMER_IDS) must not zero out the whole day's total.
            msg = str(e)
            if "permission to access customer" in msg or "USER_PERMISSION_DENIED" in msg or "PERMISSION_DENIED" in msg:
                print(f"⚠️ [GADS][{cid}] token has no access to this account — skipping it "
                      f"(remove it from GOOGLE_ADS_CUSTOMER_IDS to silence this)")
                continue
            raise
