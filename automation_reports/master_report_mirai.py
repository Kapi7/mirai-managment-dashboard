...
_GADS_CACHE_TTL_PAST_MINUTES = int(os.getenv("GOOGLE_ADS_CACHE_TTL_PAST_MINUTES", "360"))
...
# Render container path first; fall back to the file next to this script so
# local runs don't silently report Est Ship Cost = $0 (inflating Operational).
_MATRIX_PATH = os.getenv('SHIPPING_MATRIX_PATH') or (
    "/app/shipping_matrix_all.csv"
    if os.path.exists("/app/shipping_matrix_all.csv")
    else os.path.join(os.path.dirname(os.path.abspath(__file__)), "shipping_matrix_all.csv")
)
...
# Same default as monitor_orders.py — used when Shopify totalWeight is 0
...
def _shop_spend_usd(shop_orders: int, shop_gross: float,
                    ...
                    new_customer_orders: Optional[int] = None) -> float:
    ...
    # Exact spend is 0/missing. For PAST days that's trustworthy (a Shop day
    # can genuinely cost $0 when every order came from a returning customer),
    # but shop_campaign_insights lags hours behind live orders — so for TODAY,
    # with Shop orders already in, estimate billed acquisitions × latest CAC
    # instead of showing $0 spend next to real orders.
    is_today = False
    if day_iso and tz_name:
        try:
            is_today = str(day_iso)[:10] == datetime.now(pytz.timezone(tz_name)).date().isoformat()
        except Exception:
            pass
    if is_today and shop_orders > 0:
        cac = None
        try:
            import shop_campaign_cost
            ...
            pass
        billed = new_customer_orders if new_customer_orders is not None else shop_orders
        if cac and billed > 0:
            est = round(billed * cac, 2)
            print(f"[shop-cost] exact spend for {day_iso} is {exact!r} with "
                  f"{shop_orders} Shop orders — estimating {billed} acq × CAC ${cac:.2f} = ${est:.2f}")
            return est
...
def _google_spend_usd(day_iso: str, shop_tz: str) -> float:
    ...
    # Past days barely change — cache them much longer so the MTD rebuild
    # doesn't hammer the API every 30 min (each failed refetch used to flash $0).
    try:
        is_past_day = str(day_iso)[:10] < datetime.now(pytz.timezone(shop_tz)).date().isoformat()
    except Exception:
        is_past_day = False
    ttl_minutes = _GADS_CACHE_TTL_PAST_MINUTES if is_past_day else _GADS_CACHE_TTL_MINUTES

    # Check cache and TTL. Expired entries are KEPT as a stale fallback: a
    # transient Google Ads error must not turn a known spend into $0.
    ...
    if key in _GADS_CACHE:
        try:
            ...
            if isinstance(cached_entry, tuple):
                ...
                stale_value = float(cached_value)
            else:
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        # Don't cache errors — but serve the last known value instead of $0
        # so the pinned summary doesn't flash "Google: $0" on a blip.
        if stale_value is not None:
            print(f"[GADS] Serving stale cached value after error: ${stale_value:.2f}")
            return stale_value
        if not _GADS_WARNED:
...
def _kpis_from_orders(
    ...
    m_orders_created = 0
    s_orders_created = 0
    s_new_orders = 0          # Shop orders from NEW customers (the ones Shopify bills)
    shop_gross = 0.0
    ...
    for o in in_window:
        ...
        # Channel purchases count non-cancelled orders only (a cancelled order
        # is not a purchase) — matches the management dashboard's DB path.
        if ch == "google":
            g_orders_created += 1
        elif ch == "meta":
            m_orders_created += 1
        ...
        cid  = cust.get("id")
        is_returning_cust = False
        try:
            is_returning_cust = int(cust.get("numberOfOrders") or 0) > 1
        except Exception:
            pass
        if is_returning_cust and cid:
        ...
        if ch == "shop":
            ...
            if not is_returning_cust:
                s_new_orders += 1

        geo = _order_geo(o)
        wkg = _order_weight_kg_from_totalWeight(o)
        if wkg <= 0:
            # Products without configured weights → estimate items × default
            # weight instead of silently pricing the lowest matrix tier.
            _qty = sum(int(li.get("quantity") or 0) for li in _line_nodes(o))
            wkg = _qty * _DEFAULT_ITEM_WEIGHT_KG if _qty > 0 else 0.0
        est_ship = _lookup_matrix_shipping_usd(geo, wkg)
        if est_ship <= 0 and sc > 0:
            # Matrix miss (unknown GEO / no tier): $0 would read as free
            # shipping and inflate Operational — assume 80% of charged.
            est_ship = sc * 0.8
    ...
    s_spend = _shop_spend_usd(s_orders_created, shop_gross, tz_name, day_iso,
                              new_customer_orders=s_new_orders)
    ...
    # CPA over non-cancelled orders (cancelled orders inflated it before) —
    # matches the management dashboard's definition.
    general_cpa = round(total_spend / orders_net_count, 2) if orders_net_count else None
