...
# ---------------------------------------------------------------------------
# K-Beauty video concept templates (frame-by-frame for Veo 2)
# ---------------------------------------------------------------------------
...
class ContentAgent(BaseAgent):
    ...
    async def _caption_variants(self, concept: str, product: str,
                                pillar: str, channels: List[str]) -> Dict[str, str]:
        """Single AI call → channel-specific text variants."""
        brand_voice = _get_brand_voice()
        flds: List[str] = []
        if "instagram" in channels:
            flds.append('"instagram_caption":"IG caption 150-220w, educational, warm, hashtags at end"')
        if "tiktok" in channels:
            flds.append('"tiktok_caption":"TikTok 30-60w, hook first, trending hashtags"')
        if "ad" in channels:
        ...
        raw = await self.call_ai_text(prompt=prompt,
            system_prompt="Senior K-Beauty copywriter. Valid JSON only.",
    ...
    async def _build_asset(self, title: str, concept: str, product: str,
                           ...
                           txt: Dict, *, img=None, vid=None, gp=None) -> ContentAssetData:
    ...
    def _pick_concept(self, key: Optional[str], product: str) -> Dict[str, Any]:
        """Select video concept template by key or product-name heuristic."""
        if key and key in VIDEO_CONCEPT_TEMPLATES:
    ...
    def _veo2_prompt(self, tmpl: Dict[str, Any], product: str) -> str:
        ...
        return (
            ...
            f"Authentic influencer feel, not stock footage."
    ...
    async def create_social_asset(self, params: dict) -> dict:
        ...
        Params: concept, product_name, product_ids, content_pillar,
                post_type, image_engine, product_image_url."""
        ...
        await self.log_decision("create_social_asset",
            {"concept": concept, "product": product, "post_type": ptype},
            {"action": "generate_image_and_caption"},
            f"Social asset for '{concept}' via {engine}.", 0.85, False)
    ...
    async def create_ad_creative(self, params: dict) -> dict:
        ...
        Params: concept, product_name, product_ids, content_pillar,
                target_audience, ad_objective, image_engine, product_image_url."""
        ...
        await self.log_decision("create_ad_creative",
            {"concept": concept, "objective": objective},
            {"action": "generate_ad_asset"},
            f"Ad for '{concept}', audience='{audience}', obj='{objective}'.", 0.8, True)
    ...
    async def create_multi_format_asset(self, params: dict) -> dict:
        ...
        Params: concept, product_name, product_ids, content_pillar,
                video_concept_key, image_engine, product_image_url, skip_video."""
        ...
        await self.log_decision("create_multi_format_asset",
            ...
            f"Multi-format for '{concept}'. Video={'no' if no_vid else 'yes'}.", 0.85, True)

        # Text variants — single AI call
        txt = await self._caption_variants(concept, product, pillar,
                                           ["instagram", "tiktok", "ad", "blog"])
        sma = _get_social_media_agent()
        ...
        vid_result = None
        if not no_vid:
            tmpl = self._pick_concept(vkey, product)
            vp = self._veo2_prompt(tmpl, product)
        ...
        asset = await self._build_asset(
            ...
            gp={"image_engine": engine, "video_concept": vkey or "auto", "skip_video": no_vid})
        return {"asset_uuid": asset.uuid, "title": asset.title,
                "formats_generated": {
    ...
    async def create_enhanced_video(self, params: dict) -> dict:
        ...
        Params: concept, product_name, product_ids, content_pillar,
                video_concept_key, num_takes (default 2, max 3)."""
        ...
        await self.log_decision("create_enhanced_video",
            {"concept": concept, "template": tmpl["title"], "takes": takes},
            {"action": "multi_take_video"},
            f"{takes} takes via '{tmpl['title']}', best by size.", 0.75, False)

        base = self._veo2_prompt(tmpl, product)
        txt = await self._caption_variants(concept, product, pillar, ["instagram", "tiktok"])
